...
- `.mcp.json` — Claude Code / Cursor / VS Code / standard MCP clients
- `opencode.json` — OpenCode users
...
The MCP server configs (`.mcp.json`, `opencode.json`) auto-load `GEMINI_API_KEY` from `.env` — no manual export needed.
